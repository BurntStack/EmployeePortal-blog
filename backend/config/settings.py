"""
Django settings for the BurntStack Employee Portal backend.

A standalone project — auth + blog only. Configuration is environment-driven
so the same code runs across local, staging and production. Secrets live in
...
# Core
# ---------------------------------------------------------------------------
_INSECURE_DEFAULT_SECRET_KEY = "dev-insecure-key-change-me"  # nosec: dev-only fallback, guarded below
...
if not DEBUG and SECRET_KEY == _INSECURE_DEFAULT_SECRET_KEY:
    ...
    raise ImproperlyConfigured(
        "SECRET_KEY must be set to a real secret when DEBUG=False — it also "
        "signs JWTs, so the dev default would let anyone forge a valid token."
    )
...
# ---------------------------------------------------------------------------
# Static & media (WhiteNoise for static; point MEDIA at S3/Supabase Storage
# in production — local disk works for development only)
# ---------------------------------------------------------------------------
STATIC_URL = "static/"
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_THROTTLE_RATES": {
        ...
        # Stricter than the generic anon rate — this gates password guesses.
...
# ---------------------------------------------------------------------------
# CORS — the portal frontend's origin, plus the marketing site's origin so
# burntstack.com/blog can read the public post feed from here.
# ---------------------------------------------------------------------------
CORS_ALLOWED_ORIGINS = env_list(
    "CORS_ALLOWED_ORIGINS",
    "http://localhost:5173,http://127.0.0.1:5173,https://www.burntstack.com,https://burntstack.com",
)
